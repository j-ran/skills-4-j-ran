...
SELECT name, SUM(orders.num_cupcakes) FROM cupcakes LEFT JOIN orders ON (cupcakes.id = cupcake_id) GROUP BY name ORDER BY name ASC;;
...
SELECT email FROM customers LEFT JOIN orders ON(customers.id=customer_id) WHERE num_cupcakes >= 100;
...
SELECT DISTINCT fname, lname, email FROM customers JOIN orders ON(customers.id=orders.customer_id) WHERE orders.processed='True' AND orders.cupcake_id=5;